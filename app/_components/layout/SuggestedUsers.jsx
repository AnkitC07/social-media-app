        ...
        <Card style={style}>
            <div className={"max-w-[300px] w-full flex flex-col gap-6 py-3 px-5 "}>
                ...
                <div className="flex flex-col gap-5">
                    {}
                    {!loading && suggestedUsers.length > 0 ? suggestedUsers?.map((user, i) => <SuggestedUser key={i} user={user} />) :
                        <div className="py-4 pt-0 text-center text-gray-500">
                            No more people to suggest.
                    </div>
                    }
                    {loading &&
                        [0, 1, 2, 3].map((_, i) => (
